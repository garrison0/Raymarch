...
//        fadeIn = 1.0;
        ...
        float sig4 = fadeIn * (0.5 + 0.5*cos(0.1*x));
        float sig = fadeIn * (scale2 * (0.5 + 0.5*sin(0.1*x)));
        float sig2 = fadeIn * (0.5 + 0.5*sin(0.1*x + 4.0*3.141592653/3.0));
        float sig3 = fadeIn * (scale1 * (0.5 + 0.5*sin(0.1*x + 2.0*3.141592653/3.0)));
        float x2 = x / 20.0;
        float sig5 = fadeIn * scale2 * (0.5 + 0.5 * sin(0.22*x2 + sin(1.12132*x2 + cos(0.72453*x2))));
        
        float NoiseScale = 0.25*sig2 + 1.09391;
        ...
        float noise = NoiseScale * (fbm(0.5 * p3
                                      + 0.5 * sig3 * fbm(sig4*p3 + sig5*sin(0.5*p3*sig2+sin(sig4*p3+sin(0.8*sig3*p3+0.75*x))) - moveDown, 5.0)
                                      + 0.5 * sig * snoise(p3 + 0.2*moveDown + sig5*0.5*snoise(0.5*sig*3.0*p3-0.15*moveDown)),
                                    5.0) - NoiseIsoline);
...
//                pMoved *= 1.0;
                ...
                a += scale * 16.0 * (0.45 + 0.55 * (1.0 - rou));
                // these two can be divvied up with a premade array or case/switched for more detail...
                K += scale * float2(0.9, 12.0);
                f0 += scale * mat;
            }
        }
        ...
        // top light BRDF
        {
            ...
            float3 clr = 1.8*float3(1.24,1.17,1.29);
...
//            float shadow = 1.0;
            ...
            float g1SchlickVH = dot(normal, ro) / ( dot(normal, ro) * (1 - rou) + rou);
            float g1SchlickLH = dot(normal, lig) / ( dot(normal, lig) * (1 - rou) + rou);
            
            float gSmith = g1SchlickLH * g1SchlickVH;
            
            float3 spe = gSmith * dBlinnPhong * fSch / ( 4 * dot(normal, lig) * dot(normal, ro));
            ...
            lin += K.x * clr * 9.95 * spe * cshadow * cshadow; // spec
            lin += (1 - 0.5 * K.x) * 11.0 * dif * cshadow * cshadow * clr * mat * mat; // dif
            lin += 4.0*sky*K.x*float3(0.16,0.20,0.35)*occ * mat; // sky
            lin += 4.5*ind*K.x*float3(0.40,0.28,0.20)*occ * mat; // bounce
        }
        
        {
            float3 lig = float3(0.0, -1.0, -3.4);
            float3 clr = float3(1.64,1.27,0.99);
            float3 hal  = normalize(lig - rd);
            float dif = clamp(dot(normal, lig), 0.0, 1.0);
            float fre = clamp(1.0 - dot(lig, hal), 0.0, 1.0);
            float3 fSch = clamp(f0 + (float3(1) - f0)*pow(fre, 5.0), 0,1);
            lin += fSch*fre* occ*mat*clr*dif;
            lin += 0.3*fSch * clr*dif;
        }
        
        float c = smoothstep(0.8, -0.15, p.y);
                    ...
                    constant Uniforms &uniforms [[buffer(0)]])
{
    ...
    float3 color = render( ro, rd, rdx, rdy, 0.2*uniforms.time, ground, defaultSampler );
    ...
    if (uv.x > -lineLen && uv.x < lineLen && uv.y < linePos && uv.y > linePos - lineWidth) {
        total = float3(0.1);
    }
