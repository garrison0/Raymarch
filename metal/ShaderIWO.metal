...
float2 mapFlux(float3 p, float time, float3 pos) {
    ...
    // make sure to remove the bounds ('map')
    {
        time *= 1.0;
...
//        p2 *= 1.24;
        ...
        float noise = (1.2+0.25*sizeSignal(time)) * NoiseScale * (fbm( p2 - float3(0,0,-7) ) - NoiseIsoline);
        p.x -= noise;

        float d = sdCapsule(p, float3(-2.0, 0.0, 0.0), float3(-2.0,1.0,0.0)).x - ( 0.5 + 0.5 * ( sizeSignal(time)) ); // capsule // two capsules alternating?
...
float2 map (float3 p, float time) {
    ...
    res = opU(res, float2(sdBox(p, float3(100.0, .1, 100.0)), 30.0));
    ...
    res = opU(res, mapFlux(p - float3(0.58 + 2.25,4.5,-8.5), time, p+pos));
...
float waterMap ( float2 p, float time ) {
    float2x2 m2 = float2x2( 0.60, -0.80, 0.80, 0.60 );
//    float2 pm = p * m2;
    float a = 1.0 * (pow(fbm ( float3(0.29 * p*m2, time * 0.15) ), 0.4));
//    float b = 0.5 * abs ( fbm ( float3(0.099 * p, time * 0.22) ) - 0.5 ) ;
    float c = pow(1.0 * ( fbm ( float3(0.66*p*m2, time * 0.15) ) ), 5.0) ;
    return (0.9*a+0.1*c);
}

float3 getWaterNormal ( float3 pos, float time ) {
...
float3 waterColor (float3 pos, float3 rd, float3 sunLig, float time) {
    float3 col = float3(0.392, 0.583, 0.925);

    float3 normal = getWaterNormal(pos - time*float3(0.3, 0.01, 0.15), time);

    float ndotr = dot(normal, rd);
    float fresnel = pow(1.0-abs(ndotr),5.);

    float3 skyReflect = pattern(reflect(rd, normal).y, time);
...
//    col = 0.5*float3(0.996, 0.972, 0.964);
    col = pattern(rd.y, time);
...
//    time = 1.0;
    ...
    float2 texCoords = 3.25 * float2(angleBetweenXY, angleBetweenXZ); // close enough for sphere-like object
...
//        p2.y = 0;
        ...
        float3x3 mats = float3x3(ground[texIndex].sample(defaultSampler, 0.35 * pos3.yz).rgb,
                     ground[texIndex].sample(defaultSampler, 0.35 * pos3.xz).rgb,
                     ground[texIndex].sample(defaultSampler, 0.35 * pos3.xy).rgb);
        material = scale * mats * texWeights;
//        material = 0.5(material + (0.65*texture(uMetalColor, 0.35 * pos.xz + texCoords).rgb
//                                   +  0.35*texture(uMetalColor, 0.35 * pos.xy + texCoords).rgb));
        //        hNor = texture(uMetalNormal, 0.7*texCoords).rgb;
        
        
        mats = float3x3(ground[texIndex+1].sample(defaultSampler, 0.35 * pos3.yz).rgb,
                     ground[texIndex+1].sample(defaultSampler, 0.35 * pos3.xz).rgb,
                     ground[texIndex+1].sample(defaultSampler, 0.35 * pos3.xy).rgb);
        hNor = scale * mats * texWeights;

        float3 mats1x3 = float3(ground[texIndex+2].sample(defaultSampler, 0.35 * pos3.yz).r,
                      ground[texIndex+2].sample(defaultSampler, 0.35 * pos3.xz).r,
                      ground[texIndex+2].sample(defaultSampler, 0.35 * pos3.xy).r);

        rou = scale * dot(texWeights, mats1x3);
        rou = pow(rou,1.25);
//        rou = (rou - (ground[0].sample(defaultSampler, 0.35*texCoords).r));
//        material = 0.65*texture(uMetalColor, 0.35 * pos.xz + texCoords).rgb
//            +  0.35*texture(uMetalColor, 0.212 * pos.xy + texCoords).rgb;
//        hNor = texture(uMetalNormal, 0.7*texCoords).rgb;
...
//        rou = texture(uMetalRough, 0.7*texCoords).r;
    ...
    // floor
        ...
        K = float2(0.7, 4.0);
        material = pattern(rd.y, time);
        hNor = nor;
        rou = 1.0;
//        material = 1.285*float3(0.8);
    }
...
//                l2 += (1.0 - shadow) * waterColor(pos, rd, lig,  time);
                l2 += (1.0-shadow) * 0.75 * clamp(snoise(float3(0,0.01,0)*time),0.0,1.0) * clamp(waterColor(pos, rd, lig, time),0.,1.);
            }
...
//            if (m == 30) {
//                l2 += 0.5*pattern(rd.y, time) * shadow * shadow;
//            }
            
            
//            float3 lig = normalize(float3(-0.5, 0.3, 0.1));
...
//
////            float3 spe = float3(1)*(pow(clamp(dot(nor,hal), 0.0, 1.0), a / 2.0));
//            // spe *=
//
//            float3 fSch = f0 + (float3(1) - f0)*pow(fre, 5.0);
////            spe *= fSch;
...
//
//             float3 spe = (fSch * dBlinnPhong) / (4.0);
//            l+= K.x * 0.75 * spe * dif * clr;
//
//            l+= (1.0 - K.x) * 1.5 * dif * shadow * shadow * clr * material;
        }
        // back (?)
        // below - bounce
        {
...
//        float fade = smoothstep(-20.0, -30.0, pos.z);
//        float fade = 0.0;
